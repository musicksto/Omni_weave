...
test('StoryWriter instruction includes word count target', () => {
  assert.match(agentSource, /800.*1200 words/);
});
...
test('StoryReviewer validates CHARACTER SHEET', () => {
  assert.match(agentSource, /CHARACTER SHEET VALIDATION/);
});

test('StoryReviewer checks continuity', () => {
  assert.match(agentSource, /CONTINUITY/);
});
...
test('App supports video rendering in StoryPart', () => {
  assert.ok(appSource.includes("type: 'video'"));
  assert.ok(appSource.includes("<video"));
});
